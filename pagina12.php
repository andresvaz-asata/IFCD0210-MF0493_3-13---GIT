...
<main>
    ...
    <section>
        ...
        <?php
        if (!isset($_SESSION['rol']) || $_SESSION['rol'] !== 'administrador') {
            echo 'Como usuario del tipo invitado no se le permite consultar los tickets en proceso.' . '<br>';
        }
        ?>
        ...
        try {
            // Consulta para obtener todos los tickets con estado "en proceso", los nombres de los usuarios asociados y la ubicación, ordenados por fecha de creación
            $consulta = "SELECT t.id, t.fecha, t.descripcion, t.estado, u.nombre, u.apellidos, ubic.ciudad AS ubicacion FROM Ticket t
                        ...
                        WHERE t.estado = 'en proceso' ORDER BY t.fecha ASC";
            $stmt = $db->prepare($consulta);
            $stmt->execute();
            $ticketsEnProceso = $stmt->fetchAll(PDO::FETCH_ASSOC);
            ...
            if ($ticketsEnProceso) {
                ...
                echo 'No se encontraron tickets en proceso.';
            }
